...
/**
 * A {@link Codec} for {@link MonthDay} values.
 *
 * @author reed.vonredwitz
 * @since Sep-2026
 */
public class MonthDayCodec
    implements Codec<MonthDay> {

    @Override
    public Class<MonthDay> codecClass() {
    ...
    @Override
    public MonthDay decode(final JsonTransport transport,
                           final Parameter parameter,
                           final JsonValue value,
                           final Marshaller marshaller) {

        return value instanceof JsonNull ? null : MonthDay.parse(value.asString().value());
    }
}
