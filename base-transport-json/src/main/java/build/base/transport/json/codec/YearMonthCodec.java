...
/**
 * A {@link Codec} for {@link YearMonth} values.
 *
 * @author reed.vonredwitz
 * @since Sep-2026
 */
public class YearMonthCodec
    implements Codec<YearMonth> {

    @Override
    public Class<YearMonth> codecClass() {
    ...
    @Override
    public YearMonth decode(final JsonTransport transport,
                            final Parameter parameter,
                            final JsonValue value,
                            final Marshaller marshaller) {

        return value instanceof JsonNull ? null : YearMonth.parse(value.asString().value());
    }
}
